      ...
      <main className="relative z-10 max-w-6xl mx-auto px-4 pb-16">
        <article itemScope itemType="https://schema.org/Product">
          ...
          <div className="grid md:grid-cols-[280px_1fr] lg:grid-cols-[320px_1fr] gap-6 md:gap-10 mb-8">
            ...
            {/* Wine Info - Essential purchase info */}
            <div className="flex flex-col">
              ...
              {/* Vintage & Details */}
              {wine.vintage && (
                <p className="text-gold-300/60 text-sm mb-3">
              ...
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <div className="flex items-center border border-gold-700/30 rounded-lg bg-stone-900/50">
                  <button
                    ...
                    className="px-3 py-2 text-gold-300 hover:bg-gold-900/30 transition-colors rounded-l-lg"
                    aria-label="Decrease quantity"
                  >
                    -
                  </button>
                  <span className="px-3 py-2 font-medium text-gold-200">{quantity}</span>
                  <button
                    onClick={() => setQuantity(quantity + 1)}
                    className="px-3 py-2 text-gold-300 hover:bg-gold-900/30 transition-colors rounded-r-lg"
              ...
              {/* Quick Details - Visible in hero */}
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm mt-auto pt-4 border-t border-stone-700/40">
                <div>
                  <span className="text-stone-500">Producer:</span>{' '}
                  <span className="text-white">{wine.winery}</span>
                </div>
                <div>
                ...
                {wine.grape_variety && (
                  <div>
                    <span className="text-stone-500">Grape:</span>{' '}
                ...
                {wine.vintage && (
                  <div>
                    ...
                    <span className="text-white">{wine.vintage}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            {wine.tasting_notes && (
              <section className="bg-gradient-to-b from-stone-900/80 to-stone-950/90 rounded-xl p-6 border border-gold-700/20">
                ...
                <p className="text-stone-300 leading-relaxed">{wine.tasting_notes}</p>
              </section>
            )}
            ...
            <section className="bg-gradient-to-b from-stone-900/80 to-stone-950/90 rounded-xl p-6 border border-gold-700/20">
              ...
              <dl className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <dd className="text-white font-medium" itemProp="brand">{wine.winery}</dd>
                </div>
                <div>
                  <dt className="text-gold-500/60">Grape Variety</dt>
                  <dd className="text-white font-medium">{wine.grape_variety || 'Classic Blend'}</dd>
                </div>
                <div>
                  <dt className="text-gold-500/60">Wine Style</dt>
                  <dd className="text-white font-medium">{wine.style || wine.wine_type}</dd>
                </div>
                <div>
                  <dt className="text-gold-500/60">Region</dt>
                  <dd className="text-white font-medium">{wine.region}</dd>
                </div>
                <div>
                  <dt className="text-gold-500/60">Country</dt>
                  <dd className="text-white font-medium">{wine.country}</dd>
                </div>
                {wine.alcohol_percentage && (
                  <div>
                    <dt className="text-gold-500/60">Alcohol</dt>
                    <dd className="text-white font-medium">{wine.alcohol_percentage}%</dd>
                  </div>
                )}
                {wine.vintage && (
                  <div>
                    <dt className="text-gold-500/60">Vintage Year</dt>
                    <dd className="text-white font-medium">{wine.vintage}</dd>
                  </div>
                )}
              </dl>
            </section>
            ...
            <section className="bg-gradient-to-b from-stone-900/80 to-stone-950/90 rounded-xl p-6 border border-gold-700/20">
              ...
              <div itemProp="description" className="text-stone-300 leading-relaxed space-y-4">
                <p>
            ...
            {wine.food_pairings && wine.food_pairings.length > 0 && (
              <section className="bg-gradient-to-b from-stone-900/80 to-stone-950/90 rounded-xl p-6 border border-gold-700/20">
                ...
                <p className="text-stone-300 mb-4">
                  This {wine.wine_type} wine pairs beautifully with a variety of dishes:
                </p>
